...
class CustomerRealtime extends BikeGarageRealtime {
    ...
    createCustomerPanel() {
        ...
        const panelHtml = `
            <div id="customer-realtime-panel" class="bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-200 rounded-xl p-6 mb-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span id="connection-text" class="text-sm text-gray-600">מתחבר...</span>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div class="bg-white/50 rounded-lg p-4">
                        <div class="flex items-center justify-between">
                            <div>
                                <div class="text-gray-600 text-sm font-medium">התראות חדשות</div>
                                <div id="notifications-counter" class="text-2xl font-bold text-blue-600">${this.notificationsCount}</div>
                            </div>
                    ...
                    <div class="bg-white/50 rounded-lg p-4">
                        <div class="flex items-center justify-between">
                            <div>
                                <div class="text-gray-600 text-sm font-medium">תיקונים פעילים</div>
                                <div id="active-repairs-counter" class="text-2xl font-bold text-purple-600">${this.activeRepairs.length}</div>
                            </div>
            ...
            <div id="customer-notification-modal" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                <div class="bg-white rounded-xl p-6 max-w-md w-full shadow-2xl">
                    ...
                    <div class="flex gap-3">
                        ...
                        ` : ''}
                        <button onclick="customerRealtime.closeNotificationModal('${data.notification_id}')" class="flex-1 bg-gray-300 hover:bg-gray-400 text-gray-700 py-2 px-4 rounded-lg font-medium transition-colors">
                            סגור
                        </button>
                    </div>
                </div>
            ...
            <div id="ready-pickup-modal" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                <div class="bg-white rounded-xl p-6 max-w-md w-full shadow-2xl">
                    <div class="text-center mb-4">
                        ...
                        <h3 class="text-2xl font-bold text-green-800 mb-2">🎉 מוכן לאיסוף!</h3>
                        <p class="text-gray-600">התיקון הושלם בהצלחה</p>
                    </div>
                    
                    <div class="bg-green-50 border border-green-200 p-4 rounded-lg mb-6">
                        <h4 class="font-semibold text-green-800 mb-2">${data.bike_info}</h4>
                    ...
                    <div class="bg-yellow-50 border border-yellow-200 p-3 rounded-lg mb-4">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm text-yellow-800">שעות פתיחה: ראשון-חמישי 8:00-18:00</span>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-3">
                        ...
                        <button onclick="document.getElementById('ready-pickup-modal').remove()" class="flex-1 bg-gray-300 hover:bg-gray-400 text-gray-700 py-2 px-4 rounded-lg font-medium transition-colors">
                            סגור
                        </button>
                    </div>
                </div>
    ...
    addNotificationToList(notification) {
        ...
        const itemHtml = `
            <div class="notification-item bg-white border border-gray-200 rounded-lg p-4 mb-3" data-notification-id="${notification.notification_id}">
                <div class="flex items-start justify-between">
                    <div class="flex-1">
                        <h4 class="font-semibold text-gray-800">${notification.title}</h4>
                        <p class="text-sm text-gray-600 mt-1">${notification.message}</p>
                        <span class="text-xs text-gray-400 mt-2 block">${new Date().toLocaleString('he-IL')}</span>
                    </div>
    ...
    showNotificationsList() {
        ...
        const panelHtml = `
            <div id="notifications-panel" class="fixed right-4 top-20 w-96 bg-white border border-gray-200 rounded-xl shadow-xl z-40 max-h-96 overflow-y-auto">
                <div class="p-4 border-b border-gray-200">
                    <div class="flex items-center justify-between">
                        <h3 class="font-bold text-gray-800">התראות</h3>
